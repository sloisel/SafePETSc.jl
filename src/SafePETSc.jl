...
struct _Mat{T}
    ...
    """
        _Mat{T}(A::PETSc.Mat{T}, row_partition::Vector{Int}, col_partition::Vector{Int}, prefix::String)

    Inner constructor for _Mat that validates the matrix dimensions match the partitions.

    The matrix A must be fully assembled before calling this constructor.
    """
    function _Mat{T}(A::PETSc.Mat{T}, row_partition::Vector{Int},
        ...
        return new{T}(A, row_partition, col_partition, prefix)
    end
end
